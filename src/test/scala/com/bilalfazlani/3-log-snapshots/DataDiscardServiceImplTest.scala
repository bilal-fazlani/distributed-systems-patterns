...
object DataDiscardServiceImplTest extends ZIOSpecDefault { 
  val spec = suite("DataDiscardServiceImplTest")(
    test("first segment with greater value") {
      ...
      val segmentsToBeDiscarded = DataDiscardServiceImpl.segmentsToBeDiscarded(segments, 12)
      assertTrue(segmentsToBeDiscarded == Nil)
    },
    test("middle segment with greater value") {
      val segments = Array(10L, 13L, 16L, 19L)
      val segmentsToBeDiscarded = DataDiscardServiceImpl.segmentsToBeDiscarded(segments, 14)
      assertTrue(segmentsToBeDiscarded == List(10L))
    },
    test("middle segment with equal value") {
      val segments = Array(10L, 13L, 16L, 19L)
      val segmentsToBeDiscarded = DataDiscardServiceImpl.segmentsToBeDiscarded(segments, 16)
      assertTrue(segmentsToBeDiscarded == List(10L, 13L))
    },
    test("last segment with equal value") {
      val segments = Array(10L, 13L, 16L, 19L)
      val segmentsToBeDiscarded = DataDiscardServiceImpl.segmentsToBeDiscarded(segments, 19)
      assertTrue(segmentsToBeDiscarded == List(10L, 13L, 16L))
    },
    test("all non-snapshotted segments") {
      val segments = Array(10L, 13L, 16L, 19L)
      val segmentsToBeDiscarded = DataDiscardServiceImpl.segmentsToBeDiscarded(segments, 5)
      assertTrue(segmentsToBeDiscarded == Nil)
    },
    test("last segment in progress") {
      val segments = Array(10L, 13L, 16L, 19L)
      val segmentsToBeDiscarded = DataDiscardServiceImpl.segmentsToBeDiscarded(segments, 20)
      assertTrue(segmentsToBeDiscarded == List(10L, 13L, 16L))
    },
    test("no segments") {
    ...
    test("single zeroth segment") {
      ...
      val segmentsToBeDiscarded = DataDiscardServiceImpl.segmentsToBeDiscarded(segments, 0)
      assertTrue(segmentsToBeDiscarded == Nil)
    },
    test("single zeroth segment-2") {
      val segments = Array(0L)
      val segmentsToBeDiscarded = DataDiscardServiceImpl.segmentsToBeDiscarded(segments, 2)
      assertTrue(segmentsToBeDiscarded == Nil)
    }
  )
}
